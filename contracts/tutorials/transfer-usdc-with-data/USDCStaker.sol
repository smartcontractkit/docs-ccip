...
/// @title - A simple Staker contract for staking USDC tokens and redeeming the staker tokens.
contract USDCStaker is IStaker, ERC20 {
    ...
    /// @notice Stakes USDC tokens on behalf of a beneficiary, minting equivalent staking tokens.
    /// @param _beneficiary The address that will receive the minted staking tokens.
    /// @param _amount The amount of USDC to stake (in USDC's smallest unit, e.g. 1e6 = 1 USDC).
    ...
    /// @notice Redeems all staked tokens held by the caller in exchange for the equivalent USDC.
    function redeem() external {
