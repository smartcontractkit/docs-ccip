...
/// @title - A simple receiver contract for receiving USDC tokens and forwarding them to a staking contract.
contract USDCReceiver is CCIPReceiver, OwnerIsCreator {
    ...
    error MessageNotFailed(bytes32 messageId); // Used if you try to retry a message that has not failed
    ...
    function _validateSourceChain(uint64 _sourceChainSelector) internal pure {
    ...
    /// @notice Constructor initializes the contract with the router, USDC token, and staker addresses.
    ...
    constructor(address _router, address _usdcToken, address _staker) CCIPReceiver(_router) {
        ...
        i_staker = _staker;
        // Pre-approve the staker to spend USDC held by this receiver
    ...
    /// @dev Called by the OffRamp via _getCCVsFromReceiver. Reverts if the sender is not the registered sender.
        ...
        )
    {
        ...
        if (s_senders[sourceChainSelector] != decodedSender) {
            revert WrongSenderForSourceChain(sourceChainSelector);
        }
    ...
    /// @dev Extremely important to ensure only the router calls this.
    ...
    /// @dev This function must be external because of the try/catch for error handling.
    /// It uses the `onlySelf` modifier: can only be called from the contract itself.
    ...
    function _ccipReceive(Client.Any2EVMMessage memory any2EvmMessage) internal override {
        ...
        // Decode the beneficiary from the message data; use the actual received token amount
        // (destTokenAmounts[0].amount) rather than the sender-encoded amount to account for
        // any CCTP transfer fees applied on the destination side.
        // ABI layout of any2EvmMessage.data: [4-byte selector][32-byte beneficiary][32-byte amount]
        // In memory: data pointer + 32 (length word) + 4 (selector) = offset 36 for beneficiary slot.
        address beneficiary;
        {
            bytes memory d = any2EvmMessage.data;
            assembly {
                beneficiary := mload(add(d, 36))
            }
        }
        uint256 actualAmount = any2EvmMessage.destTokenAmounts[0].amount;

        // Low-level call to the staker contract with the actual received amount
        // bytes4(keccak256("stake(address,uint256)")) = 0xadc9772e
        (bool success,) = i_staker.call(
            abi.encodeWithSelector(bytes4(keccak256("stake(address,uint256)")), beneficiary, actualAmount)
        );
    ...
    function retryFailedMessage(bytes32 messageId, address beneficiary) external onlyOwner {
        ...
        // Transfer the associated tokens to the specified beneficiary as an escape hatch.
    ...
    /// @param offset The index of the first failed message to return.
    /// @param limit The maximum number of failed messages to return.
    /// @return failedMessages An array of `FailedMessage` structs representing the requested subset of failed messages.
    function getFailedMessages(uint256 offset, uint256 limit) external view returns (FailedMessage[] memory) {
        ...
        for (uint256 i = 0; i < returnLength; i++) {
            ...
            failedMessages[i] = FailedMessage({messageId: messageId, errorCode: ErrorCode(errorCode)});
        }
