...
async function main() {
  ...
  console.log("");
  console.log("========================================");
  console.log("🚀 Deploy CCIP Messenger Contracts on Both Chains");
  console.log("========================================");
  ...
  console.log(`\n[Step 1] Deploying Messenger on ${sourceConfig.chainName}`);
  const {
  ...
  console.log(`\n[Step 2] Deploying Messenger on ${destConfig.chainName}`);
  const {
  ...
  console.log(
    `SOURCE_CHAIN=${sourceChainName} DEST_CHAIN=${destChainName} ALLOWED_FINALITY_CONFIG=BLOCK_DEPTH ALLOWED_BLOCK_DEPTH=10 npx hardhat run hardhat/scripts/tutorials/send-arbitrary-data/configure/configure.ts`
  );
  console.log("");
  console.log("For bidirectional messaging (send from destination back to source), also run:");
  console.log(
    `SOURCE_CHAIN=${destChainName} DEST_CHAIN=${sourceChainName} ALLOWED_FINALITY_CONFIG=BLOCK_DEPTH ALLOWED_BLOCK_DEPTH=10 npx hardhat run hardhat/scripts/tutorials/send-arbitrary-data/configure/configure.ts`
  );
  console.log("========================================");
  console.log("");
