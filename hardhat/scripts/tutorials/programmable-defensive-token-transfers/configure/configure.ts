...
const SIM_REVERT = (process.env.SIM_REVERT ?? "true").toLowerCase() === "true";
...
async function main() {
  ...
  console.log("");
  console.log("========================================");
  console.log("⚙️ Configure CCIP Defensive Contracts on Both Chains");
  console.log("========================================");
  ...
  console.log(`Setting simRevert to ${SIM_REVERT}...`);
  const setSimRevertHash = await destWalletClient.writeContract({
    address: destContract,
    abi: artifact.abi,
    functionName: "setSimRevert",
    args: [SIM_REVERT],
    account: destAccount,
    chain: destChain,
  });
  ...
  await destPublicClient.waitForTransactionReceipt({
    hash: setSimRevertHash,
    confirmations: destConfig.confirmations,
  });
  console.log(`✅ Sim revert set to ${SIM_REVERT} - messages will ${SIM_REVERT ? "fail for testing recovery" : "be processed normally"}`);
  ...
  if (SIM_REVERT) {
    console.log("⚠️  Messages will fail on destination (simRevert=true) to demonstrate defensive handling");
  }
  console.log("");
  ...
  console.log(
    `SOURCE_CHAIN=${sourceChainName} DEST_CHAIN=${destChainName} FEE_TOKEN=LINK TOKEN_AMOUNT=1000000000000000 GAS_LIMIT=400000 ${finalityHint} MESSAGE='Hello World From Hardhat Script for CCIP 2.0!' npx hardhat run hardhat/scripts/tutorials/programmable-defensive-token-transfers/interact/send-message.ts`
  );
  console.log("");
  console.log("Or send a message paying with native gas (default):");
  console.log(
    `SOURCE_CHAIN=${sourceChainName} DEST_CHAIN=${destChainName} TOKEN_AMOUNT=1000000000000000 GAS_LIMIT=400000 ${finalityHint} MESSAGE='Hello World From Hardhat Script for CCIP 2.0!' npx hardhat run hardhat/scripts/tutorials/programmable-defensive-token-transfers/interact/send-message.ts`
  );
  console.log("========================================");
  console.log("");
}
