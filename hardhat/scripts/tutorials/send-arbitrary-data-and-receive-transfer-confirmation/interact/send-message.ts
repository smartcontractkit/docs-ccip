...
const GAS_LIMIT = process.env.GAS_LIMIT ? Number(process.env.GAS_LIMIT) : 500_000;
...
async function main() {
  ...
  console.log("");
  console.log("========================================");
  console.log(`📡 CCIP Message (A→B→A) - Pay with ${feeTokenSymbol}`);
  console.log("========================================");
  ...
  const extraArgs = await buildMessageOnlyExtraArgs({
    ...
    sender: messageTrackerAddress,
    receiver: acknowledgerAddress,
    gasLimit: GAS_LIMIT,
  ...
  if (payingWithErc20) {
    console.log(`\n[Step ${step++}] Approving MessageTracker to spend ${feeTokenSymbol} for CCIP fees...`);
    ...
    const approveFeeHash = await walletClient.writeContract({
      ...
      args: [messageTrackerAddress, ccipFee],
      account,
      chain,
    });
    ...
    console.log(`✅ MessageTracker approved to spend ${feeTokenSymbol}`);
    console.log("");
  }

  if (!payingWithErc20) {
    console.log(
      `\n[Step ${step}] Sending message via MessageTracker with native fee (${sourceConfig.nativeCurrencySymbol})...`
    );
    console.log(`Required CCIP fee (in WEI): ${ccipFee}`);
  } else {
    console.log(`\n[Step ${step}] Sending message via MessageTracker...`);
  }
  ...
  const sendHash = payingWithErc20
    ? await walletClient.writeContract({
        ...
        args: [BigInt(destConfig.chainSelector), acknowledgerAddress, MESSAGE, feeToken, extraArgs],
        account,
        chain,
      })
    : await walletClient.writeContract({
  ...
  console.log("");
  console.log("Track message status (1 = Sent, 2 = ProcessedOnDestination):");
  console.log(
    `SOURCE_CHAIN=${sourceChainName} MESSAGE_ID=${messageId} npx hardhat run hardhat/scripts/tutorials/send-arbitrary-data-and-receive-transfer-confirmation/interact/get-message-status.ts`
  );
  console.log("========================================");
  console.log("");
}
