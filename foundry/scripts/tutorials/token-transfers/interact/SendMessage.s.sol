...
/// @notice Sends a CCIP token-only transfer from the source chain.
///
...
contract SendMessage is ExtraArgsHelper {
    ...
    function run() external {
        ...
        require(receiver != address(0), "Receiver address not set. Set RECEIVER_ADDRESS env var");
        ...
        if (params.payingWithErc20) {
            ...
            console.log(string.concat(unicode"📡 CCIP Token Transfer - Pay with ", tokenSymbol));
        } else {
            console.log(unicode"📡 CCIP Token Transfer - Pay with Native");
        }
        console.log("========================================");
        ...
        _executeTransfer(sourceContract, receiver, sourceConfig, destConfig, params, sourceChainId);
    }
    ...
    function _executeTransfer(
        ...
        bytes memory extraArgs = buildExtraArgs(
            ...
            0, // gasLimit = 0 for token-only transfers (no receiver callback)
            params.requestedFinalityConfig
        );

        // Calculate required CCIP fee
        uint256 ccipFee = TokenTransferor(sourceContract)
            .getFee(
                destConfig.chainSelector,
                receiver,
                sourceConfig.ccipBnM,
        ...
        if (params.payingWithErc20) {
            console.log(string.concat("\n[Step ", vm.toString(step), "] Sending CCIP token transfer..."));
            bytes32 messageId = TokenTransferor(sourceContract)
                .sendMessage(
                    destConfig.chainSelector,
                    receiver,
                    sourceConfig.ccipBnM,
            ...
            console.log(
                string.concat("\n[Step ", vm.toString(step), "] Sending CCIP token transfer with native token fee ("),
                helperConfig.getNativeCurrencySymbol(sourceChainId),
                ")..."
            );
            console.log("Required CCIP fee (in WEI):", ccipFee);
            bytes32 messageId = TokenTransferor(sourceContract).sendMessage{value: ccipFee}(
                destConfig.chainSelector, receiver, sourceConfig.ccipBnM, params.ccipBnmAmount, address(0), extraArgs
            );
            vm.stopBroadcast();
            _logSuccess(messageId);
        }
    ...
    function _logSuccess(bytes32 messageId) internal view {
        console.log("");
        console.log("========================================");
        console.log(unicode"✅ Tokens transferred successfully!");
        console.log("========================================");
